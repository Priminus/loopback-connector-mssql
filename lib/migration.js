...
function mixinMigration(MsSQL) {
  ...
  MsSQL.prototype.propertiesSQL = function(model) {
    ...
    var pks = this.idNames(model);

    var sql = [];
    var props = Object.keys(objModel.properties);
    for (var i = 0, n = props.length; i < n; i++) {
      var prop = props[i];
      if (pks.length === 1 && prop === pks[0]) {
    ...
    if (pks.length === 1) {
      ...
      cmd += ' ' + self.columnEscaped(model, pks[0]) + ' ASC' + MsSQL.newline;
      cmd += ') WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, ' +
      'IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON)';
    } else if(pks.length > 1) {
      var pksEscaped = this.idNames(model).map(function(i) {
        return self.columnEscaped(model, i);
      });
      cmd = 'PRIMARY KEY' + MsSQL.newline + '(' + MsSQL.newline;
      cmd += ' ' + pksEscaped.join(',') + MsSQL.newline;
      cmd += ')';
    }
